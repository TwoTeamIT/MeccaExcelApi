...
namespace DucatiExcelApi.Controllers
{
    ...
    public class Mecca : ControllerBase
    {
        ...
        private readonly Sql_Engine _sqlEngine;
        private readonly ILogger<Mecca> _logger;
        private Stopwatch _stopwatch;

        public Mecca(IConfiguration configuration, ILogger<Mecca> logger)
        {
            var connStr = (configuration as IConfiguration)?.GetConnectionString("DefaultConnection");

            if (string.IsNullOrEmpty(connStr))
                throw new InvalidOperationException("La stringa di connessione 'DefaultConnection' non è stata trovata o è nulla.");

            _sqlEngine = new Sql_Engine(connStr);

            _logger = logger;
            _stopwatch = new Stopwatch();
        }

        [HttpGet("Getefn_ARTICOLI_Xml")]
        public IActionResult GetFunctionXml([FromQuery] string Distinta)
        {
            Response.ContentType = "application/xml; charset=utf-8";
            Response.Headers.Add("Cache-Control", "no-cache");
            Response.Headers.Add("Transfer-Encoding", "chunked");

            DataTable dt = _sqlEngine.EseguiFunzioneJson("efn_ARTICOLI", new List<SqlParameter> { new SqlParameter("@Distinta", Distinta) });
            dt.TableName = "efn_ARTICOLI";
            ...
            Console.WriteLine("Inizio XmlWriteMode...");
            ...
            Console.WriteLine($"Fine XmlWriteMode - Tempo trascorso: {elapsedTime}");

            stream.Position = 0;

            return File(stream, "application/xml", $"{efn_ARTICOLI}.xml");
        }



        [HttpGet("StreamFunction")]
        [AllowAnonymous]
        public async Task efn_ARTICOLI([FromQuery] string Distinta)
        {
            try
            {
                await _sqlEngine.StreamFunctionAsJson(Response, "efn_ARTICOLI",
                    new List<SqlParameter> { new SqlParameter("@Distinta", Distinta) });
            }
            catch (Exception ex)
            {
